...
namespace Microsoft.VisualStudio.Project
{
    ...
    public class BuildableProjectConfig : IVsBuildableProjectCfg
    {
        ...
        protected virtual void Build(uint options, IVsOutputWindowPane output, string target)
        {
            ...
            try
            {
                config.ProjectManager.Build(options, this.config.ConfigName, this.config.Platform, output, target, (result, buildTarget) => this.NotifyBuildEnd(result, buildTarget));
            }
            catch(Exception e)
            {
