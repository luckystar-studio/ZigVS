...
#nullable enable
    ...
    internal sealed class QAndACommand
    {
        static QAndACommand? s_Instance;

        private QAndACommand(OleMenuCommandService? i_OleMenuCommandService)
        {
            if (i_OleMenuCommandService != null)
            {
                ...
                var l_CommandID = new CommandID(CommandDefinition.s_CommandSetGuid, (int)CommandDefinition.CommandId.QAndA);
        ...
        public static Task InitializeAsync()
        {
            ...
            s_Instance = new QAndACommand(commandService);
        ...
        private void Execute(object i_senderObject, EventArgs i_EventArgs)
        {
            ...
            try
            {
                string l_URLString = "https://marketplace.visualstudio.com/items?itemName=LuckystarStudio.ZigVS&ssr=false#qna";

                if (string.IsNullOrEmpty(l_URLString))
                {
                    string l_messageString = string.Format(CultureInfo.CurrentCulture, "Could not find Q & A page: " + l_URLString, this.GetType().FullName);
                    string l_titleString = "Q & A";
