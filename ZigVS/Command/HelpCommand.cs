...
#nullable enable
    ...
    {
        static HelpCommand? s_Instance = null;

        private HelpCommand(OleMenuCommandService? i_OleMenuCommandService)
        {
            if (i_OleMenuCommandService != null)
            {
                ...
                var l_CommandID = new CommandID(CommandDefinition.s_CommandSetGuid, (int)CommandDefinition.CommandId.Help);
        ...
        public static Task InitializeAsync()
        {
            ...
            s_Instance = new HelpCommand(commandService);
        ...
        private void Execute(object i_senderObject, EventArgs i_EventArgs)
        {
            ...
            try
            {
                string l_URLString = "https://luckystar-studio.github.io/ZigVS-Web/";

                if (string.IsNullOrEmpty(l_URLString))
                {
                    string l_messageString = string.Format(CultureInfo.CurrentCulture, "Could not find help page: " + l_URLString, this.GetType().FullName);
